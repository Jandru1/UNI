...
;;;;;;;;;;;;;;;
;; APORTACIONS QUE HEM FET: ;;
;; L'Extending model d'aquest projecte consistia en 4 coses: Afegir nivells, afegir bonus, afegir sistema de puntuació i programar un control automàtic. I hem fet lo següent: ;;
;; -Hem creat 3 nivells més. El nivell 6, 7 i 8. Com el joc portava una dinàmica de que quant més gran sigui el nivell més complicat és, hem fet els nivells més complicats del joc ;;
;; -Hem creat un bonus-live, és a dir, un element que al agafar-ho s'incrementa el nombre de vides en 1 unitat (obviament només es pot agafar un cop) ;;
;; -Hem creat un element el qual, si l'agafes i arribes a la meta (el pad) t'incrementa el nombre de punts (obviament només es pot agafar un cop) ;;
;; -Hem creat un element el qual, si l'agafes i arribes a la meta (el pad) t'incrementa el nombre de punts. Es poden agafar sempre es torni a començar la partida;;
;; -Hem implementat un sistema automàtic el qual fa que la granota es mogui sempre i quan ho pugui fer sense risc. De totes formes, aquesta implementació no sempre aconsegueix fer arribar a la granota al pads, ja que hi han 2 situacions on no es mor: ;;
;;     1) Quan no té on anar: Als logs (fustas a l'aigua) quan la granota no pot anar a cap direcció i aquest arriba al limit (ja que no es deixa de moure) ;;
;;     2) Quan just el moment exacte en que es mou la granota, es mou també un dels vehicles i colisionen ;;
;;     3) Quan és a les tortugues de riu i justament la tortuga a la que està s'enfonsa (és una funcionalitat del model) ;;
;;     Aquesta funcionalitzat es pot utilitzar prement la tecla A, i es veurà a gran velocitat si la tecla es manté pressionada ;;
;;     Recomenem no utilitzar-la sense pensar, sino premer la tecla quan calgui per a que la granota no fassi moviments innecesaris ;;
;;;;;;;;;;;;;;;
...
to setup              ;; Initializes the game
  ...
  set score 0
  set gotpoint? false
  set can-move-up? false
  set can-move-down? false
  set can-move-right? false
  set can-move-left? false
  set is-safe? false
  set passedWithPoints? false
  reset-ticks
end
...
to next-level        ;; This will call the appropriate level procedure, where the level is created
  ...
  if (level = 8 )
    [ level-8 ]
  if (level = 9 )
    [ user-message "Bro, if you passed the last level you can die in peace "
      set dead? true]
end
    ...
    [ sprout-trucks 1
        [ set color truck-color
          ...
          ifelse ((pxcor = x) xor (direction = 90))
            [ set shape "truck" ] ;; aqui le da formas que tiene netlogo
            [ set shape "truck rear" ]
        ]
    ]
end

to create-star [x y]     ;; Creates and initializes a star
  create-stars 1
    [ set color 45
      setxy x y
      set shape "star"
...
to create-point [x y]     ;; Creates and initializes a star
  create-points 1
    [ set color 15
      setxy x y
      set shape "target"
...
to check-frog
  ask frogs
    [ if any? pads-here with [shape = "pad"]
      ...
      [increment-lives]
      if ( any? points-here )
      [ask points-here [ hide-turtle]
      gotpointTrue]
    ]
  if ( pads-done = 5 )
    [ set level (level + 1)
      set pads-done 0
      ifelse(passedWithPoints?)[user-message (word "Congrats, all your frogs are safe and you got the points!\nOn to level " level "...")]
...
to kill-frog
  ask points [set hidden? false]
  ;; This is called when the frog dies, checks if the game is over
  set lives (lives - 1)
  ifelse (lives = 0)
...
to automatic
  ...
  set can-move-right? false
 ask frogs [
    ifelse (([pcolor] of patch-at 0 1 != blue and not any? trucks-on patch-at 0 1 and not any? cars-on patch-at 0 1 ) or
      ([pcolor] of patch-at 0 1 = blue and any? logs-on patch-at 0 1 or any? river-turtles-on patch-at 0 1 or
        (any? (pads-on patch-at 0 1) with [shape = "pad"])))
    [
      set can-move-up? true
      output-print("up")

    ][
   ifelse ((([pcolor] of patch-at -1 0 != blue and not any? trucks-on patch-at -1 0 and not any? cars-on patch-at -1 0) or
        ([pcolor] of patch-at -1 0 = blue and any? logs-on patch-at -1 0 or any? river-turtles-on patch-at -1 0 or any? pads-on patch-at -1 0)) and
    ([pxcor] of patch-at -1 0 != pxcor))
      [
      set can-move-left? true
      output-print("left")
    ][
    ifelse ((([pcolor] of patch-at 1 0 != blue and not any? trucks-on patch-at 1 0 and not any? cars-on patch-at 1 0) or
          ([pcolor] of patch-at 1 0 = blue and any? logs-on patch-at 1 0 or any? river-turtles-on patch-at 1 0 or any? pads-on patch-at 1 0  )) and
      ([pxcor] of patch-at 0 -1 != pxcor))
          [
            set can-move-right? true
            output-print("right")
       ]
          [
          if ((([pcolor] of patch-at 0 -1 != blue and not any? trucks-on patch-at 0 -1 and not any? cars-on patch-at 0 -1) or
          ([pcolor] of patch-at 0 -1 = blue and any? logs-on patch-at 0 -1 or any? river-turtles-on patch-at 0 -1 or any? pads-on patch-at 1 0 )) and
             ([pycor] of patch-at 0 -1 != pycor))
              [
            set can-move-down? true
            output-print("down")
            ]]
      ]
    ]
...
to level-6
 create-truck -8 -5 270 .2
  create-truck -2 -5 270 .2
  create-truck 6 -5 270 .2
  create-truck 4 -4 90 .2
  create-truck -1 -4 90 .2
  create-truck -6 -4 90 .2
  create-car -4 -3 270 .2
  create-car 0 -3 270 .2
  create-car 4 -3 270 .2
  create-car 7 -3 270 .2
  create-car -3 -2 90 .2
  create-car -5 -2 90 .2
  create-car 5 -2 90 .2
  create-car 1 -2 90 .2
  create-car 0 -1 270 .2
  create-car 5 -1 270 .2
  create-car -7 -1 270 .2
  create-car -3 -1 270 .2
  create-log -3 3 3 .2
  create-log -3 5 3 .2
  create-log -3 7 3 .2
  create-star 0 0
  create-point -2 -3
  create-river-turtle -4 4 4 .2
  create-river-turtle 4 4 4 .2
  create-river-turtle -7 4 1 .2
  create-river-turtle -1 6 3 .2
  create-river-turtle -8 6 3 .2
  create-river-turtle 3 6 2 .2
  make-river-turtle-dive 5
end


to level-7
  create-car -10 -5 270 .2
  create-car -4 -5 270 .2
  create-car 4 -5 270 .2
  create-car 2 -4 90 .2
  create-car -3 -4 90 .2
  create-car -8 -4 90 .2
  create-truck -6 -3 270 .2
  create-truck -2 -3 270 .2
  create-truck 2 -3 270 .2
  create-truck 5 -3 270 .2
  create-truck -5 -2 90 .2
  create-truck -7 -2 90 .2
  create-truck 3 -2 90 .2
  create-truck -1 -2 90 .2
  create-truck -5 -1 270 .2
  create-truck 3 -1 270 .2
  create-truck -8 -1 270 .2
  create-truck -1 -1 270 .2
  create-log -5 3 3 .2
  create-log -5 5 3 .2
  create-log -5 7 3 .2
  create-star -5 -4
  create-star 5 -5
  create-point -2 -3
  create-river-turtle -6 4 4 .2
  create-river-turtle 2 4 4 .2
  create-river-turtle -9 4 1 .2
  create-river-turtle -3 6 3 .2
  create-river-turtle -10 6 3 .2
  create-river-turtle 1 6 2 .2
  make-river-turtle-dive 5
end


to level-8
 create-truck -6 -5 270 .1
  create-truck 0 -5 270 .1
  create-truck 6 -5 270 .1
  create-truck 5 -4 90 .1
  create-truck 1 -4 90 .1
  create-truck -4 -4 90 .1
  create-truck -2 -3 270 .1
  create-truck 2 -3 270 .1
  create-truck 6 -3 270 .1
  create-truck 9 -3 270 .1
  create-truck 0 -2 90 .1
  create-truck -3 -2 90 .1
  create-truck 7 -2 90 .1
  create-truck 3 -2 90 .1
  create-truck 2 -1 270 .1
  create-truck 7 -1 270 .1
  create-truck -5 -1 270 .1
  create-truck -1 -1 270 .1
  create-log -3 3 1 .2
  create-log -3 5 1 .2
  create-log -3 7 1 .2
  create-point -8 -5
  create-star 0 -3
  create-point 8 -1
  create-river-turtle -4 4 2 .1
  create-river-turtle 4 4 2 .1
  create-river-turtle -7 4 2 .1
  create-river-turtle -1 6 2 .1
  create-river-turtle -8 6 2 .1
  create-river-turtle 3 6 2 .1
...
D'acord a l'Extending Model, hem afegit els seguents canvis:

-Nivell 6, 7 i 8 . Cadasqun més complicat que l'anterior.

-Life-Bonus: Bonificació amb forma d'estel. Incrementa en 1 unitat el nombre de vides.

-Point-Bonus: Bonificació de puntuació. Incrementa en 100 el nombre de punts.

-Automatic movement: La granota es mourà automàticament allà on estigui fora de perill.
	(Ordre de preferències: Up, Left, Right, Down)

Més informació a dins del codi.
