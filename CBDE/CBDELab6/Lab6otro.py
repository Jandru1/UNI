...
def create_part(session, identifier, partkey, mfgr, type, size):
    session.run("CREATE (" + identifier + ":Part {partkey:'" + partkey +
                "', mfgr:'" + mfgr + "', type: '" + str(type) + "', size: " + size + "})")
...
def create_order(session, identifier, orderkey, orderdate, shippriority, c_marketsegment, n_name):
    session.run("CREATE (" + identifier + ":Order {orderkey: '" + orderkey + "', orderdate: '" + str(orderdate) + "' , shippriority:'" +
...
def create_lineitem(session, identifier, orderkey, suppkey, returnflag, quantity,
    ...
    session.run("CREATE (" + identifier + ":LineItem {orderkey: '" + orderkey +
                ...
                ", shipdate: '" + str(shipdate) + "', linestatus: '" + linestatus + "'})")
...
def create_edge_supplier_part(session, supplier, suppkey, part, partkey, supplycost):
    session.run(
        ...
        "'}) CREATE (" + supplier + ")-[:ps {supplycost: '" + str(supplycost) + "'}]->(" + part + ")")
...
def create():
    ...
    db = GraphDatabase.driver("bolt://localhost:7687", auth=basic_auth("neo4j", "CBDE"))
    return inserts(db)
...
def query1(db, date):
    ...
    result = \
        db.session().run(" MATCH " +
                         ...
                         "      l_returnflag       ASC, " +
                         "      l_linestatus       ASC ",
                         parameters={"date": str(time.mktime(date.timetuple()))})
...
def query3(db, date1, date2, segment):
    ...
    result = \
        db.session().run(" MATCH " +
                         ...
                         "      revenue        DESC, " +
                         "      o_orderdate    ASC ",
                         parameters={"date1": str(time.mktime(date1.timetuple())),
                          "date2": str(time.mktime(date2.timetuple())),
                          "segment": segment})
...
def query4(db, region, date):
    ...
    result = \
        db.session().run(" MATCH " +
                         ...
                         "      revenue     DESC ",
                         parameters={"date": str(time.mktime(date.timetuple())),
                          "date_year": str(time.mktime(date.replace(year=date.year + 1).timetuple())),
                          "region": region})
    i = 0
    for item in result:
        i += 1
