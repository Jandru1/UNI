 ...
 */
#define PLAYER_NAME Jandruns10_
...
bool alguien_lleva_x_alimento(int n, int worker) {
      ...
      for (int k = 0; k < int(perm.size()); ++k) {
        ...
        if (pos_ok(ant(worker_id).pos) and ant(worker_id).bonus == alimento and worker_id != worker) return true;
      }
      return false;
}
...
int distancia_Soldier_enemigo(Position start) {
  ...
  while (not Q.empty()) {
      ...
      p2.i = p.r;
      p2.j = p.c;
      if (pos_ok(p2) and cell(p2).id != -1 and ant(cell(p2).id).type != Soldier and ant(cell(p2).id).player != me()) return p.d;
...
void Queen_takes_food(int queen_id, Pos p, bool &ha_comido, NoSuicidisPlz &NoSuicidis){
  ...
  if(not ha_comido){
    if(pos_ok(p+Up+Right) and cell(p+Up+Right).bonus != None) {
      if(pos_ok(p+Up) and cell(p+Up).type != Water and cell(p+Up).id == -1 and not posicio_perillosa(p + Up, queen_id, NoSuicidis)) {
        ha_comido = true;
        move(queen_id, Up);
        No_Suicidis_plz(NoSuicidis, Up, p);
        }
      else if(pos_ok(p+Right) and cell(p+Right).type != Water and cell(p+Right).bonus == -1 and not posicio_perillosa(p + Right, queen_id, NoSuicidis)) {
        ha_comido = true;
        move(queen_id, Right);
      ...
      else if(pos_ok(p+Left)and cell(p+Left).type != Water and cell(p+Left).bonus == -1 and not posicio_perillosa(p + Left, queen_id, NoSuicidis)) {
        ha_comido = true;
        move(queen_id, Left);
      ...
      if(pos_ok(p+Down)and cell(p+Down).type != Water and cell(p+Down).id == -1 and not posicio_perillosa(p + Down, queen_id, NoSuicidis)) {
        ha_comido = true;
        move(queen_id, Down);
      ...
      if(pos_ok(p+Left)and cell(p+Left).type != Water and cell(p+Left).id == -1 and not posicio_perillosa(p + Left, queen_id, NoSuicidis)) {
        ha_comido = true;
        move(queen_id, Left);
        No_Suicidis_plz(NoSuicidis, Left, p);
        }
      else if(pos_ok(p+Down) and cell(p+Down).type != Water and cell(p+Down).bonus == -1 and not posicio_perillosa(p + Down, queen_id, NoSuicidis)) {
        ha_comido = true;
        move(queen_id, Down);
        No_Suicidis_plz(NoSuicidis, Down, p);
        }
...
bool valid_position_soldiers (const Visitadesx &V, Pos p) {
  return (pos_ok(p) and cell(p).type != Water and not V[p.i][p.j] and ((cell(p).id != -1  and ant(cell(p).id).type == Worker and ant(cell(p).id).player != me() ) or cell(p).id == -1));
}
...
bool es_el_worker_con_menos_vida(int worker_id){
  vector<int>my_worker_ids = workers(me());
  int worker_min = -1;
  for (int k = 0; k < int(my_worker_ids.size()); ++k) {
    if (k == 0) worker_min = my_worker_ids[k];
    else {
      int worker_id2 = my_worker_ids[k];
      if ((ant(worker_id2).life) < (ant(worker_min).life)) worker_min = worker_id;
    }
  }
  if(worker_id == worker_min) return true;
  else return false;
}

Dir dir_inversa(Dir d, NoSuicidisPlz &NoSuicidis, Pos p, int worker_id) {
  for(int i = 0; i < 4; ++i) {
    Pos aux = p + Dir(i);
    if(Dir(i) != d and pos_ok(aux) and cell(aux).type != Water and not posicio_perillosa(aux,worker_id, NoSuicidis) and not posicio_perillosa_enemics(aux,worker_id)) {
      return Dir(i);
    }
  }
  return Dir(4);
      ...
      aux = actual + Dir(i);
       if (pos_ok(aux) and valid_position_soldiers(Visitades, aux)) {
        if (prim){
          ...
          Direccions[aux.i][aux.j] = Direccions[actual.i][actual.j];
          /*if (not posicio_perillosa_soldiers(aux,soldier_id))*/ posicions.push(aux);
        }
...
Dir bfs_workers_for_food (int worker_id, Pos p, const NoSuicidisPlz &NoSuicidis) {
    ...
    Visitades[p.i][p.j] = true;
    if (worker_adjacent(p, worker_id)){
       if(not tiene_mas_workers_que_yo(p, Dir(dir_worker_adjacent(p, worker_id)))) return Dir(dir_worker_adjacent(p, worker_id));
     }
    int f = posicions.front().i;
    int c = posicions.front().j;
    ...
    for (int k = 0; k < int(perm.size()); ++k) {
      ...
      if (c.bonus != None) {
        if (valid_position_closetoQueen(p)) {
          ...
          if(n >= 0 and es_el_worker_con_menos_vida(worker_id) and perm.size() > 1 and not alguien_lleva_x_alimento(n, worker_id)) {
          ...
          if(ant(cell(p).id).life >= dist) kamikaze(p, V, worker_id);
            ...
            No_Suicidis_plz(V,d,p);
              ...
              Position Position_Worker(p.i,p.j,0);
              int dist = distancia_Soldier_enemigo(Position_Worker);
              if(dist <= 3){
                cout << "tengo un soldier cerca y la distancia es " << dist << endl;
                Dir d = bfs_soldiers_kamikaze(worker_id, p, V);
                Dir d2 = dir_inversa(d, V, p, worker_id);
                move(worker_id, d2);
                No_Suicidis_plz(V,d2,p);
              }
            }
          }
        }
