 ...
 */
#define PLAYER_NAME JandretGVNG
...
bool alguien_lleva_x_alimento(int alimento) {
      ...
      for (int k = 0; k < int(perm.size()); ++k) {
        ...
        if (pos_ok(ant(worker_id).pos) and cell(ant(worker_id).pos).bonus == alimento) return true;
      }
      return false;
}
...
Dir bfs_workers_for_x_alimento(int worker_id, Pos p, const NoSuicidisPlz &NoSuicidis, int n){
  ...
  Visitades[p.i][p.j] = true;
  if (worker_adjacent(p, worker_id)) return Dir(dir_worker_adjacent(p, worker_id));
    ...
    for (int k = 0; k < int(perm.size()); ++k) {
      ...
      if (c.bonus != None) {
        if (valid_position_closetoQueen(p)) {
          if(em_falta_x_aliment(queen_id) and k == 0 and perm.size() != 1) {
            int n = que_aliment_em_falta(queen_id);
            if (not alguien_lleva_x_alimento(n)) {
              BonusType alimento;
              if(n == 0) alimento = Bread;
              else if (n == 1) alimento = Leaf;
              else alimento = Seed;
              Dir d = bfs_workers_for_x_alimento(worker_id, p, V, alimento);
              move(worker_id, d);
              No_Suicidis_plz(V,d,p);
            }
          }
